...
def lista_usuario():
    ...
    cursor.execute('SELECT * FROM usuarios_cloud')
...
def login():
    if request.method == 'POST':
        ...
        db = mysql.connector.connect(**db_config2)
        cursor = db.cursor(dictionary=True)
        cursor.execute("SELECT * FROM usuarios_cloud WHERE nome = %s AND senha = %s", (username, password))
...
def register():
    if request.method == 'POST':
        ...
        db = mysql.connector.connect(**db_config2)
        cursor = db.cursor()
        cursor.execute("INSERT INTO usuarios_cloud (nome, email, senha) VALUES (%s, %s, %s)", (username, email, password))
        db.commit()
        cursor.close()
        db.close()
...
def formulario(id=None):
    ...
    if id:
        db = mysql.connector.connect(**db_config2)
        cursor = db.cursor(dictionary=True)
        cursor.execute("SELECT * FROM usuarios_cloud WHERE ID = %s", (id,))
        user = cursor.fetchone()
        cursor.close()
        db.close()
    ...
    if request.method == 'POST':
        ...
        if id:
            cursor.execute("UPDATE usuarios SET nome = %s, email = %s, senha = %s WHERE isusuario_cloud = %s", (nome, email, senha, id))
...
def delete(id):
    ...
    db = mysql.connector.connect(**db_config2)
    cursor = db.cursor()
    cursor.execute("DELETE FROM usuarios_cloud WHERE ID=%s", (id,))
    db.commit()
    cursor.close()
    db.close()
    return redirect(url_for('usuario.dashboard'))
